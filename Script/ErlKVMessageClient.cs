...
public class ErlKVMessageClient
{
	private string _cmd;
	...
	public ErlKVMessageClient(string cmd)
	{
		this._cmd = cmd;
	...
	{
		int number = ConnectCount.getInstance().number;
		this.addValue(null, new ErlInt(number));
		this.bytesWriteKey(data, this._cmd);
		this.bytesWriteInfo(data);
		Log.Info ("+++++++++++"+ this._cmd + "|"+  this.toJsonString()+ "+++++++++++");
	}
